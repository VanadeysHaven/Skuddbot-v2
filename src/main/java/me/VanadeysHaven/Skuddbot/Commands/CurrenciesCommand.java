...
public class CurrenciesCommand extends Command {
    ...
    public void run(CommandRequest request) {
        ...
        if(args.length >= 2 && MiscUtils.isLong(args[1]) && UserUtils.getInstance().doesUserExist(args[1]))
            user = UserUtils.getInstance().getUser(args[1]);

        PermissionManager authorPerms = pm.getUser(server.getIdLong(), author.getIdLong()).getPermissions();
        ...
        if(user.getIdLong() != author.getIdLong() && su.getSettings().getBoolean(UserSetting.PROFILE_PRIVATE) && !authorPerms.hasPermission(PermissionLevel.SERVER_ADMIN)){
            MessagesUtils.addReaction(message, Emoji.X, "This user has set their currencies to private.");
            return;
        }

        EmbedBuilder eb = new EmbedBuilder();
        eb.setAuthor("Currencies for: " + UserUtils.getDisplayName(server, user), null, user.getEffectiveAvatarUrl());
        eb.setTitle("__Server:__ " + server.getName());

        for(Currency currency : Currency.values())
            if(currency.isShowWhenZero() || su.getCurrencies().getInt(currency) > 0)
                eb.addField("__" + currency.getName() + ":__", su.getCurrencies().getString(currency) + " " + currency.getSuffix(), true);

        MessagesUtils.sendEmbed(request.getChannel(), eb);
    }

    private void editValue(Message message, String content, SkuddUser su, User user, Guild server) {
        ...
        try {
            switch (args[3].toLowerCase()) {
                case "add":
                    ...
                    MessagesUtils.addReaction(message, Emoji.WHITE_CHECK_MARK, "Added `" + mutationAmount + "` to currency `" + currency + "` for user `" + UserUtils.getDisplayName(server, user) + "`");
                    break;
                case "remove":
                    su.getCurrencies().incrementInt(currency, mutationAmount * -1);
                    MessagesUtils.addReaction(message, Emoji.WHITE_CHECK_MARK, "Removed `" + mutationAmount + "` from currency `" + currency + "` for user `" + UserUtils.getDisplayName(server, user) + "`");
                    break;
                case "set":
                    su.getCurrencies().setInt(currency, mutationAmount);
                    MessagesUtils.addReaction(message, Emoji.WHITE_CHECK_MARK, "Set currency `" + currency + "` to `" + mutationAmount + "` for user `" + UserUtils.getDisplayName(server, user) + "`");
                    break;
                default:
